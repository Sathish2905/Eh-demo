...
public class RemoteCache2 {
	private static final Logger LOGGER = getLogger(RemoteCache2.class);

	private static Cache<Long, CAOCache> remoteCache = null;

	public static void main(String[] args) {
		URL myUrl = RemoteCache2.class.getResource("/ehcache.xml");
		...
		try (CacheManager cacheManager = newCacheManager(xmlConfig)) {
			...
			remoteCache.getRuntimeConfiguration().registerCacheEventListener(
					...
					EnumSet.of(EventType.CREATED, EventType.UPDATED,
							EventType.REMOVED));
			for (int i = 0; i < 10; i++) {
				CAOCache ch = new CAOCache(Long.valueOf(i), "WEB-2", "Obj" + i);
